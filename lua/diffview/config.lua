...
-- Concatenate multiple keymap tables into one.
local function compose_keymaps(...)
  local result = {}
  for i = 1, select("#", ...) do
    local t = select(i, ...)
    for _, km in ipairs(t) do result[#result + 1] = km end
  end
  return result
end
...
M.defaults = {
  ...
  keymaps = {
    ...
    view = compose_keymaps(common_nav_keymaps, {
    ...
    file_panel = compose_keymaps(common_panel_keymaps, common_nav_keymaps, {
    ...
    file_history_panel = compose_keymaps(common_panel_keymaps, common_nav_keymaps, {
