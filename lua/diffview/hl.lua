...
--
-- `sp` is set equal to `bg` as a sentinel attribute. Neovim's compositor
-- silently drops a `hl_group` extmark whose spec contains only `bg` when it
-- lands over a `line_hl_group` extmark -- the overlay never reaches the
-- screen and changed chars render with the underlying line bg (visible on
-- e.g., tokyonight, where `DiffText` is bg-only). Any second attribute
-- unblocks composition; `sp` is the cheapest because no decoration is
-- active to actually paint it, and matching `bg` keeps it invisible even
-- if a downstream group ever activates underline/undercurl.
